...
namespace GSLanguageCompiler::IO {
    ...
    std::unique_ptr<GS_InFileStream> GS_InFileStream::Create(std::ifstream stream, GSStreamInfoPtr streamInfo) {
        return std::make_unique<GS_InFileStream>(std::move(stream), std::move(streamInfo));
    }

    std::unique_ptr<GS_InFileStream> GS_InFileStream::CreateInFile(UString name) {
    ...
    std::unique_ptr<GS_InConsoleStream> GS_InConsoleStream::Create(LRef<std::istream> stream, GSStreamInfoPtr streamInfo) {
        return std::make_unique<GS_InConsoleStream>(stream, std::move(streamInfo));
    }

    std::unique_ptr<GS_InConsoleStream> GS_InConsoleStream::CreateCIn() {
    ...
    std::unique_ptr<GS_OutFileStream> GS_OutFileStream::Create(std::ofstream stream, GSStreamInfoPtr streamInfo) {
        return std::make_unique<GS_OutFileStream>(std::move(stream), std::move(streamInfo));
    }

    std::unique_ptr<GS_OutFileStream> GS_OutFileStream::CreateOutFile(UString name) {
    ...
    std::unique_ptr<GS_OutConsoleStream> GS_OutConsoleStream::Create(LRef<std::ostream> stream, GSStreamInfoPtr streamInfo) {
        return std::make_unique<GS_OutConsoleStream>(stream, std::move(streamInfo));
    }

    std::unique_ptr<GS_OutConsoleStream> GS_OutConsoleStream::CreateCOut() {
        return GS_OutConsoleStream::Create(std::cout, GS_StreamInfo::CreateConsoleInfo());
    }

    std::unique_ptr<GS_OutConsoleStream> GS_OutConsoleStream::CreateCErr() {
        return GS_OutConsoleStream::Create(std::cerr, GS_StreamInfo::CreateConsoleInfo());
    }

    std::unique_ptr<GS_OutConsoleStream> GS_OutConsoleStream::CreateCLog() {
