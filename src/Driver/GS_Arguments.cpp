...
namespace GSLanguageCompiler::Driver {
    ...
    GS_Arguments::GS_Arguments(std::vector<UString> inputFileNames, UString outputFileName)
            : _inputFileNames(std::move(inputFileNames)), _outputFileName(std::move(outputFileName)) {}

    GS_Arguments GS_Arguments::Create(std::vector<UString> inputFileNames, UString outputFileName) {
        return GS_Arguments(std::move(inputFileNames), std::move(outputFileName));
    }
    ...
    GS_Arguments GS_Arguments::Create() {
        return GS_Arguments::Create(std::vector<UString>(), UString());
    }
    ...
    std::optional<GS_Arguments> GS_Arguments::Create(I32 argc, Ptr<Ptr<C>> argv) {
        ...
        try {
            ...
            GS_GlobalContext::Out(parser.Help());

            return std::nullopt;
        } catch (LRef<args::Error> error) {
            GS_GlobalContext::Out(UString(error.what()) + UString("\n\n"));
        ...
        return GS_Arguments::Create(inputFileNames, outputFileName);
    }
